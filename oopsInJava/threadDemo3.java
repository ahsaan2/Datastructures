...
class thread1 extends Thread {
    ...
    public void run() {
        for (int i = 0; i < 5; i++) {
            ...
            // once the first statement is printed, we can make it wait for some seconds, to
...
public class threadDemo3 {
    public static void main(String[] args) {
        ...
        // start is used for starting the thread, and it calls the run method of the
        // Thread class
        thread2 obj2 = new thread2();

        // Now if we want to set the high priority for the thread 2
        obj2.setPriority(10); // highest priotiy
        obj1.start(); // this will start the thread and it will call the run method.

        obj2.start(); // calls the run method of the thread 2
        // here we can see the output of both the threads in parallel, executing one
        // after another
        // because of the try block where we sleep the threads for some time, after the
        // first execution.

        // get the priority of the thread1
        System.out.println(obj1.getPriority()); // should print 5 because threads by default have the priority of 5.
