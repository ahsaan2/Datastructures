...
public class Heap<T extends Comparable<T>> {
  ...
  // ------------------------HEAPSORT----------------------------------------
  // we have a heap, till the heap is empty keep removing items, and put it in the
  // list.
  ...
  public static void main(String[] args) throws Exception {
    ...
    // priorityQueue-->> we have priority such as (max item(maxHeap) and min
    // item(minHeap)).
  }
}
/**
 * why heaps-->> heaps are the data structures that we use when we want to be
 * able of access the minimum
 * or the maximum element very quickly.
 * Suppose we have to find the smallest item, what do we do ... we store the
 * items in an array but
 * every time we add an item , we sort the array. By this way the smalles item
 * will be found in O(1)
 * time.
 * //The insertion in worst case is taking O(N logn) time, because in the worst
 * case you indert an item and it is less
 * then the item at index 0.This complexity is redued by the use of heaps.
 * --------------------------------------HOW HEAPS
 * WORK------------------------------------------------------------------
 * -->> Internally heaps will stored as an array but it will be represented as a
 * tree.
 * ----------------------How heap is
 * represented-------------------------------------
 * 1. In reality it will be stored in an array, we will read it like a tree.
 * 2. Every node will have value >= all of its children.
 * 3. The max item or the min item will be at the very first index, the rest of
 ...
 * 1. while inserting an item, check it with its parent, if the item is smaller
 * than its parent then swap.
 * this will make sure that the smalles item will be at the index = 0.
 * --------------------------------------REMOVING ITEMS FROM THE
 * HEAP-----------------------------------------------------
 * 1. Take the last item and put it into the zeroth index, and search from
 * downwards, check what is smaller, left is smaller or
 * right is smaller. left is smaller swap it.
 * -----------insertion/ deletion of items becomes O(log n)in
 * heaps-------------------------------------------------------
 *
 * 
 */
